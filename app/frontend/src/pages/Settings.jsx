...
function S50CdrSection() {
    ...
    return (
      <div className="card">
        <div className="alert error">
          {message?.text || 'Unable to load PBX CDR settings.'}
        </div>
  ...
  return <div className="card"><h3 style={{marginTop:0}}>PBX CDR</h3><p className="muted">Used only to import CDRs and retrieve recordings. This application does not place calls or control the PBX.</p>{message&&<div className={`alert ${message.type}`}>{message.text}</div>}<form onSubmit={save}><div className="form-grid">
    <div><label>Enable PBX CDR</label><select value={form.enabled?'yes':'no'} onChange={e=>update('enabled',e.target.value==='yes')}><option value="no">Disabled</option><option value="yes">Enabled</option></select></div>
    <div><label>PBX Server / IP *</label><input value={form.server||''} onChange={e=>update('server',e.target.value)} placeholder="192.168.x.x" required /></div>
  ...
  </div><div className="actions"><button type="submit" className="primary" disabled={saving}>{saving?'Saving…':'Save PBX settings'}</button><button type="button" className="secondary" onClick={test} disabled={testing||!form.server||!form.api_username}>{testing?'Testing…':'Test Connection'}</button></div></form>{form.last_sync_at&&<div className="field-help" style={{marginTop:'1rem'}}>Last sync: {String(form.last_sync_at).replace('T',' ').slice(0,19)} · {form.last_sync_status||'—'} · {form.last_sync_message||''}</div>}</div>;
}
