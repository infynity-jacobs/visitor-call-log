...
function S50CdrView() {
  ...
  async function sync() {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `S50 sync complete: ${data.result.fetched} fetched, ${data.result.inserted} new, ${data.result.updated} updated.` });
      await load(0);
    } catch (err) { setMessage({ type: 'error', text: err.message }); }
  ...
  return <>
    ...
    <div className="card s50-filter-card">
      <div className="section-heading"><div><h3 style={{ margin: 0 }}>S50 CDR</h3><div className="muted">Yeastar S50 call records and recordings. Inspection only — no calling or PBX control.</div></div><button className="primary" onClick={sync} disabled={syncing}>{syncing ? 'Syncing…' : '↻ Sync S50'}</button></div>
    ...
    <div className="card">
      ...
      <div className="table-wrap">
        <table className="mobile-cards s50-cdr-table">
          ...
          </tr>)}{!records.length && <tr><td colSpan="9" style={{ color:'#6b7280' }}>{busy ? 'Loading…' : 'No S50 CDR records found. Use Sync S50 after configuring the PBX.'}</td></tr>}</tbody>
        </table>
      </div>
